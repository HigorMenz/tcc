  ...
  return (
    <>
      <nav className="navbar ">
        ...
        <div
          className={
            "absolute bg-bg  w-full left-0 top-full mt-0.5 border-b  border-grey  border-opacity-20 md:border-0 md:mt-0 py-4 px-[5vw] md:block md:relative md:inset-0 md:p-0 md:w-auto md:opacity-100 md:pointer-events-auto duration-200 " +
            (searchBoxVisible ? "show" : "hide")
          }
        >
          <input
            type="text"
            className="w-full md:w-auto  bg-grey p-2 pl-6 pr-[12%] md:pl-12 md:pr-6 rounded-full placeholder:text-dark-grey"
          ...
          {access_token ? (
            <>
              <Link to="/dashboard/notifications">
                <button className="w-12 h-12 rounded-full bg-grey relative hover:bg-grey/80">
                  <i className="fi fi-rr-bell text-2xl block mt-2"></i>
                  {userAuth ? (
                    new_notifications_available ? (
                      <span className="bg-red w-3 h-3 rounded-full absolute z-20 top-2 right-2"></span>
                    ) : (
                      ""
                    )
                  ) : (
